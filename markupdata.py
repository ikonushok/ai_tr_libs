...
class MarkUpData():
  ...
  def use_optimizer(self, col_name, optimizer='Adam', **opt_params):
    ...
    opt = {'Adam': Adam, 'AdadeltaBigStart': AdadeltaBigStart, 'AdadeltaZeroStart': AdadeltaZeroStart, 'RMSProp': RMSProp}[optimizer]
    try:
  ...
  def mark_bbands(self, ticks=None, timeperiod=5, nbdevup=2, nbdevdn=2, matype=0):
    ...
    if not ticks:
      ticks = self.high
    try:
      self.data['upperband'], self.data['middleband'], self.data['lowerband'] = talib.BBANDS(ticks, timeperiod, nbdevup, nbdevdn, matype)
      return 200
    except Exception as error:
      return str(error)
    ...
    try:
      for col_1 in col_names:
        for col_2 in col_names:
          if col_1 == col_2:
            continue
          self.data[f'{col_1}_{col_2}'] = np.round(activation_func(self.data[f'{col_1}'] - self.data[f'{col_2}']), round_to)
      return 200
    except Exception as error:
      return str(error)
  ...
  def cmp_col_to_cols(self, col_name, col_names, activation='tanh', round_to=0):
    ...
    try:
      for col in col_names:
        self.data[f'{col_name}_{col}'] = np.round(activation_func(self.data[f'{col_name}'] - self.data[f'{col}']), round_to)
      return 200
    except Exception as error:
      return str(error)
  ...
  def calc_row_min(self, col_name, col_names, activation='line', absolut=True, round_to=10):
    ...
    activation_func = self.activation_funcs[activation]
    try:
      self.data[f'{col_name}_min'] = self.data.apply(lambda x: np.round(np.abs(activation_func(x[col_names].min())), round_to), axis=1)
      return 200
    except Exception as error:
      return str(error)
  ...
  def calc_row_max(self, col_name, col_names, activation='line', absolut=True, round_to=10):
    ...
    activation_func = self.activation_funcs[activation]
    try:
      self.data[f'{col_name}_max'] = self.data.apply(lambda x: np.round(np.abs(activation_func(x[col_names].max())), round_to), axis=1)
      return 200
    except Exception as error:
      return str(error)
